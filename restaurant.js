...
class Positions {
  ...
  /*  positionAmount() {
        return this.arrayEmployee.length;
    }*/
    ooo(){
        let counter = 0;
...
addEmployee("Финансовый", "Главный бухгалтер", 311, 'Елена', 'Попова', 28, '20.06.2021', "" );
...
if(restaurant[i].title === department){
        ...
        for(let j = 0; j < restaurant[i].arrayPositions.length; j++){
            ...
            sum += restaurant[i].arrayPositions[j].salaryAmount() * restaurant[i].arrayPositions[j].ooo();
            console.log(sum);
        }
}
...
console.log(salaryDepartment("Финансовый"));
